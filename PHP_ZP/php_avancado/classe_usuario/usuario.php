...
class Usuario {
    ...
    public function __construct($i = '') {      // = '' transforma variável passada como parâmetro em uma variável 'opçional'
        try {
    ...
    public function salvar() {      // método 'salvar'
        if(!empty($this-> id)) {        // se foi encontrado um 'id'
            $sql = "UPDATE usuarioss SET email = ?, senha = ?, nome = ? WHERE id = ?";       // query de 'atualização'
            $sql = $this-> pdo-> prepare($sql);
            $sql-> execute(array($this-> email, $this-> senha, $this-> nome, $this-> id));      // mandando as informações do contato
        } else {
            
            $sql = "INSERT INTO usuarioss SET email = ?, senha = ?, nome = ?";
            $sql = $this-> pdo-> prepare($sql);
            $sql-> execute(array($this-> email, $this-> senha, $this-> nome));
        }
    }

    public function delete() {      // método de 'deletar' usuário
        $sql = "DELETE FROM usuarioss WHERE id = ?";
        $sql = $this-> pdo-> prepare($sql);
